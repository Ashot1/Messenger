...
const DropDownMenu: FC<IDropDownMenu> = ({list, children, dopClassWrapper, needArrow = true, dopClassList}) => {
	...
	return (
		<button className={`${styles.Wrapper} ${dopClassWrapper}`} onBlur={() => setTimeout(() => setOpenState(false), 250)}>
			<div className={`${styles.CurrentValute} ${OpenState ? styles.CurrentValueActive : ''}`}
				 onClick={() => setOpenState(prev => !prev)}>
				<div className={styles.TopMenu}>
					{children}
					{needArrow && <motion.span animate={{rotate: OpenState ? 0 : 180}} className={styles.Arrow}></motion.span>}
				</div>
			</div>
			<AnimatePresence>
				{OpenState &&
