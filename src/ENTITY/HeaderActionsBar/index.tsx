    ...
    if (user.notifications) return (
        <div className={styles.actions}>
            ...
            <UserCircle url={user.userPhoto || userPNG}
                        ...
                        style={{
                            opacity: OpenState.user ? '.5' : '1',
                            filter: !user.userPhoto ? 'var(--invertFilter)' : ''
                        }}
            ...
            <ModalBlock openState={OpenState.notif} dopClass={styles.NotifModal}>
                            ...
                            return <li key={index} onClick={() => {
                                ...
                                {SystemsNotifications.includes(notif.icon)
                                    ...
                                    : <img src={src || userPNG} alt="фото" className={styles.UserImg}
                                           style={{filter: !src ? 'var(--invertFilter)' : ''}}/>}
                ...
                {user.notifications.length >= 1
                    && <div className={styles.ButtonPosition}>
						<BorderedButton click={() => clearNotifications(user)} BGColor="var(--MainColor)"
						                color="var(--InvertMainColor)" reversed>
							Очистить все
						</BorderedButton>
					</div>}
            </ModalBlock>
